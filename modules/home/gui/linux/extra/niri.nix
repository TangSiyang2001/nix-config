...
let
  wallpapers = "${pkgs.mypkgs.assets}/share/wallpapers";
in
{

  config = lib.mkIf config.custom.linux.desktop.wm.niri.enable {
    ...
    programs.niri.settings = {
      ...
      binds = with config.lib.niri.actions; {
        ...
        "Mod+Shift+B" = {
          action = spawn-sh "pkill -USR1 waybar";
          hotkey-overlay.title = "Toggle bar";
        };
        "Mod+slash" = {
          action = spawn-sh "thunar ~/.xdg";
          hotkey-overlay.title = "Open file manager: thunar";
        };
        "Mod+semicolon" = {
          action = spawn-sh "walker -m clipboard";
